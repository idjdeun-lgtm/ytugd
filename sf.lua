...
local settingsTable = {
	General = {
		...
		-- buildwarnings
		-- rayfieldprompts
		keybindOptions = {Type = 'toggle', Value = false, Name = 'Keybind Options'},
	},
	System = {
...
local function getIcon(name : string)
	...
	local r = Icons[name] or (Icons['48px'] and Icons['48px'][name]) or (Icons['all'] and Icons['all'][name])
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" then
		if id:find("rbxassetid://") or id:find("http") or id:find("asset") then
			return id
		end
	end
	return "rbxassetid://0"
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			task.spawn(function()
				...
				elseif Flag.Type == "Toggle" and type(FlagValue) == "table" and FlagValue.Keybind ~= nil then
					changed = true
					Flag:Set(FlagValue.Value)
					if Flag.SetKeybind then
						Flag:SetKeybind(FlagValue.Keybind)
					else
						Flag.Keybind = FlagValue.Keybind
						if Flag.KeybindButton then
							Flag.KeybindButton.Text = tostring(FlagValue.Keybind)
							Flag.KeybindButton.TextColor3 = SelectedTheme.TextColor
						end
						if Flag.ClearKeybindButton then
							Flag.ClearKeybindButton.Visible = true
						end
					end
				else
...
local function SaveConfiguration()
	...
	for i, v in pairs(RayfieldLibrary.Flags) do
		...
		elseif v.Type == "Toggle" and v.Keybind then
			Data[i] = {Value = v.CurrentValue, Keybind = v.Keybind}
		else
...
function RayfieldLibrary:Notify(data)
	task.spawn(function()
		...
		-- Setup References before reparenting
		local Title = newNotification.Title
		local Description = newNotification.Description
		local Icon = newNotification.Icon

		-- Move elements to MainFrame to avoid occlusion and fix layout
		...
		-- Dynamic Layout Correction
		Title.TextXAlignment = Enum.TextXAlignment.Left
		Description.TextXAlignment = Enum.TextXAlignment.Left
		
		local hasIcon = (data.Image ~= nil)
		local padding = hasIcon and 45 or 15
		
		Title.Position = UDim2.new(0, padding, 0, 10)
		Title.Size = UDim2.new(1, -(padding + 15), 0, 20)
		
		Description.Position = UDim2.new(0, padding, 0, 32)
		Description.Size = UDim2.new(1, -(padding + 15), 0, 30)
		...
		Description.Text = data.Content or "Notification Content"
		...
		if newNotification:FindFirstChild("UIStroke") then newNotification.UIStroke.Color = SelectedTheme.TextColor end
		Icon.ImageColor3 = SelectedTheme.TextColor
		
		-- Typography Polish
		Title.Font = Enum.Font.GothamBold
		Description.LineHeight = 1.1
		...
		ProgressBar.Position = UDim2.new(0, 2, 1, -5) -- Offset by 2px from bottom and sides
		ProgressBar.Size = UDim2.new(1, -4, 0, 3)
		ProgressBar.ZIndex = 3
		ProgressBar.Parent = MainFrame

		local ProgressCorner = Instance.new("UICorner")
		ProgressCorner.CornerRadius = UICorner.CornerRadius
		...
		if newNotification:FindFirstChild("UIStroke") then newNotification.UIStroke.Transparency = 1 end
		if newNotification:FindFirstChild("Shadow") then newNotification.Shadow.ImageTransparency = 1 end
		Icon.ImageTransparency = 1
		newNotification.Visible = true

		local bounds = {Title.TextBounds.Y, Description.TextBounds.Y}
		...
		TweenService:Create(Title, TweenInfo.new(0.5, Enum.EasingStyle.Cubic), {TextTransparency = 0}):Play()
		TweenService:Create(Description, TweenInfo.new(0.5, Enum.EasingStyle.Cubic), {TextTransparency = 0.2}):Play()
		TweenService:Create(Icon, TweenInfo.new(0.5, Enum.EasingStyle.Cubic), {ImageTransparency = 0}):Play()
		...
		local duration = data.Duration or math.min(math.max((#MainFrame.Description.Text * 0.05) + 3, 3), 10)
...
local function openSearch()
	...
	local SearchBar = Main.Search
	SearchBar.BackgroundTransparency = 1
	SearchBar.Shadow.ImageTransparency = 1
	SearchBar.Input.TextTransparency = 1
	SearchBar.Search.ImageTransparency = 1
	SearchBar.UIStroke.Transparency = 1
	
	SearchBar.Input.Font = Enum.Font.GothamBold
	SearchBar.Input.PlaceholderText = "Search features..."
	SearchBar.Input.PlaceholderColor3 = SelectedTheme.TextColor
	
	-- Improved Search Icon
	if Icons then
		local asset = getIcon("search")
		SearchBar.Search.Image = 'rbxassetid://'..asset.id
		SearchBar.Search.ImageRectOffset = asset.imageRectOffset
		SearchBar.Search.ImageRectSize = asset.imageRectSize
	end

	SearchBar.Size = UDim2.new(1, -20, 0, 80)
	SearchBar.Position = UDim2.new(0.5, 0, 0, 70)
	SearchBar.Input.Interactable = true
	SearchBar.Visible = true

	for _, tabbtn in ipairs(TabList:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "Placeholder" then
			tabbtn.Interact.Visible = false
			TweenService:Create(tabbtn, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
			TweenService:Create(tabbtn.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 1}):Play()
			TweenService:Create(tabbtn.Image, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 1}):Play()
			TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
			if tabbtn:FindFirstChild("Indicator") then
				TweenService:Create(tabbtn.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
			end
		end
	end
	...
	TweenService:Create(SearchBar.Shadow, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0.9, ImageColor3 = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)}):Play()
	TweenService:Create(SearchBar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Position = UDim2.new(0.5, 0, 0, 57), BackgroundTransparency = 0.05, BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	TweenService:Create(SearchBar.UIStroke, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Transparency = 0, Color = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)}):Play()
	TweenService:Create(SearchBar.Input, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {TextTransparency = 0.1}):Play()
	TweenService:Create(SearchBar.Search, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {ImageTransparency = 0.3, ImageColor3 = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)}):Play()
	TweenService:Create(SearchBar, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -35, 0, 35)}):Play()
end
...
local function closeSearch()
	...
	local SearchBar = Main.Search
	TweenService:Create(SearchBar, TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.In), {BackgroundTransparency = 1, Size = UDim2.new(1, -55, 0, 30), Position = UDim2.new(0.5, 0, 0, 70)}):Play()
	TweenService:Create(SearchBar.Search, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {ImageTransparency = 1}):Play()
	TweenService:Create(SearchBar.Shadow, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {ImageTransparency = 1}):Play()
	TweenService:Create(SearchBar.UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {Transparency = 1}):Play()
	TweenService:Create(SearchBar.Input, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {TextTransparency = 1}):Play()

	for _, tabbtn in ipairs(TabList:GetChildren()) do
		if tabbtn.ClassName == "Frame" and tabbtn.Name ~= "Placeholder" then
			tabbtn.Interact.Visible = true
			if tostring(Elements.UIPageLayout.CurrentPage) == tabbtn.Title.Text then
				TweenService:Create(tabbtn, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
				TweenService:Create(tabbtn.Image, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0}):Play()
				TweenService:Create(tabbtn.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()
				TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
				if tabbtn:FindFirstChild("Indicator") then
					TweenService:Create(tabbtn.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
				end
			else
				TweenService:Create(tabbtn, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0.7}):Play()
				TweenService:Create(tabbtn.Image, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {ImageTransparency = 0.2}):Play()
				TweenService:Create(tabbtn.Title, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {TextTransparency = 0.2}):Play()
				TweenService:Create(tabbtn.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Transparency = 0.5}):Play()
				if tabbtn:FindFirstChild("Indicator") then
					TweenService:Create(tabbtn.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
				end
			end
	...
	SearchBar.Input.Interactable = false
	task.wait(0.4)
...
local function Hide(notify: boolean?)
	...
	-- Fix: Ensure content doesn't overflow during close animation
	Main.ClipsDescendants = true

	if useMobilePrompt and MPrompt then
		TweenService:Create(MPrompt, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, 50, 0, 50), Position = UDim2.new(0.5, 0, 0, 40), BackgroundTransparency = 0.2}):Play()
		TweenService:Create(MPrompt.Title, TweenInfo.new(0.4, Enum.EasingStyle.Cubic), {TextTransparency = 0}):Play()
		if MPrompt:FindFirstChild("Glow") then
			TweenService:Create(MPrompt.Glow, TweenInfo.new(0.6, Enum.EasingStyle.Cubic), {ImageTransparency = 0.5}):Play()
		end
	end
...
local function Unhide()
	...
	if MPrompt then
		TweenService:Create(MPrompt, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.5, 0, 0, -50), BackgroundTransparency = 1}):Play()
		TweenService:Create(MPrompt.Title, TweenInfo.new(0.4, Enum.EasingStyle.Cubic), {TextTransparency = 1}):Play()
		if MPrompt:FindFirstChild("Glow") then
			TweenService:Create(MPrompt.Glow, TweenInfo.new(0.4, Enum.EasingStyle.Cubic), {ImageTransparency = 1}):Play()
		end
...
local function createSettings(window)
	...
	for categoryName, settingCategory in pairs(settingsTable) do
		...
		for settingName, setting in pairs(settingCategory) do
			...
			elseif setting.Type == 'toggle' then
				local isKeybindOption = (settingName == 'keybindOptions')
				if isKeybindOption and UserInputService.TouchEnabled then
					setting.Value = false
				end
				...
				setting.Element = newTab:CreateToggle({
					...
					Callback = function(Value)
						if isKeybindOption and UserInputService.TouchEnabled then
							if Value == true then
								-- Force disable if mobile tries to enable
								setting.Element:Set(false)
								RayfieldLibrary:Notify({
									Title = "Unavailable",
									Content = "Keybind options are not available on mobile devices.",
									Image = 4400697855, -- Warning icon
								})
								return
							end
						end
						...
						-- Global update for toggle keybind visibility
						if settingName == 'keybindOptions' then
							for _, toggleFlags in pairs(RayfieldLibrary.Flags) do
								-- Check if it's a toggle with a KeybindButton
								if type(toggleFlags) == "table" and toggleFlags.KeybindButton then
									toggleFlags.KeybindButton.Visible = Value
								end
							end
						end
					end,
				...
				if isKeybindOption and UserInputService.TouchEnabled then
                     ...
                     -- The logic above forces it off if they try to click it.
				end
	...
	-- System info section
	newTab:CreateSection("System Information")
	
	local executorLabel = newTab:CreateLabel("Executor: " .. (identifyexecutor and identifyexecutor() or "Unknown"))
	local fpsLabel = newTab:CreateLabel("FPS: 0")
	local pingLabel = newTab:CreateLabel("Ping: 0ms")
	local uptimeLabel = newTab:CreateLabel("Uptime: 0s")
	...
	task.spawn(function()
		...
		while true do
			...
			if currentTime - lastUpdate >= 1 then
				...
				uptimeLabel:Set(string.format("Uptime: %dh %dm %ds", hours, mins, secs))
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			-- Only override if no saved setting exists to ensure persistence
			local currentSaved = getSetting("General", "rayfieldOpen")
			if currentSaved == "K" or currentSaved == nil then
	...
	if MPrompt then
		MPrompt.Title.Text = Settings.ShowText or "Show"
		MPrompt.Size = UDim2.new(0, 0, 0, 0)
		MPrompt.Position = UDim2.new(0.5, 0, 0, -50)
		MPrompt.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
		
		local MPromptCorner = Instance.new("UICorner")
		MPromptCorner.CornerRadius = UDim.new(1, 0)
		MPromptCorner.Parent = MPrompt
		
		local MPromptGradient = Instance.new("UIGradient")
		MPromptGradient.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)),
			ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 40, 40))
		})
		MPromptGradient.Rotation = 45
		MPromptGradient.Parent = MPrompt

		local MPromptStroke = Instance.new("UIStroke")
		MPromptStroke.Color = Color3.fromRGB(255, 255, 255)
		MPromptStroke.Transparency = 0.8
		MPromptStroke.Thickness = 1.5
		MPromptStroke.Parent = MPrompt

		local MPromptGlow = Instance.new("ImageLabel")
		MPromptGlow.Name = "Glow"
		MPromptGlow.BackgroundTransparency = 1
		MPromptGlow.Position = UDim2.new(0, -15, 0, -15)
		MPromptGlow.Size = UDim2.new(1, 30, 1, 30)
		MPromptGlow.Image = "rbxassetid://6015667362"
		MPromptGlow.ImageColor3 = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)
		MPromptGlow.ImageTransparency = 1
		MPromptGlow.ZIndex = MPrompt.ZIndex - 1
		MPromptGlow.Parent = MPrompt

		MPrompt.Title.Font = Enum.Font.GothamBold
		MPrompt.Title.TextSize = 12
		
		MPrompt.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				TweenService:Create(MPrompt, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Size = UDim2.new(0, 42, 0, 42)}):Play()
			end
		end)
		
		MPrompt.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				TweenService:Create(MPrompt, TweenInfo.new(0.4, Enum.EasingStyle.Back), {Size = UDim2.new(0, 50, 0, 50)}):Play()
			end
		end)
	end
	...
	if not Settings.DisableRayfieldPrompts then
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "NEGROO",
					Content = "t juif bah join bueno pd",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	pcall(function()
		...
		-- FORCE ENABLE to ensure keybinds are saved
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		-- Tab Indicator and Glow (Gen Aesthetic)
		...
		Glow.Position = UDim2.new(0.5, -20, 1, -12)
		Glow.Size = UDim2.new(0, 40, 0, 20)
		Glow.Image = "rbxassetid://6015667362"
		Glow.ImageColor3 = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)
		...
		if Elements.UIPageLayout.CurrentPage == TabPage then
			...
			TabIndicator.Size = UDim2.new(0.6, 0, 0, 3)
		...
		TabButton.Interact.MouseButton1Click:Connect(function()
			...
			TweenService:Create(TabIndicator, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0.6, 0, 0, 3), BackgroundTransparency = 0}):Play()
			TweenService:Create(Glow, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {ImageTransparency = 1}):Play()
			...
			for _, OtherTabButton in ipairs(TabList:GetChildren()) do
				if OtherTabButton.Name ~= "Template" and OtherTabButton.ClassName == "Frame" and OtherTabButton ~= TabButton and OtherTabButton.Name ~= "Placeholder" then
					...
					if OtherTabButton:FindFirstChild("Indicator") then
						TweenService:Create(OtherTabButton.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, 0, 0, 3), BackgroundTransparency = 1}):Play()
					end
				end
			end
		...
		TabButton.Interact.MouseEnter:Connect(function()
			if Elements.UIPageLayout.CurrentPage ~= TabPage then
				...
				TweenService:Create(Glow, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {ImageTransparency = 0.5}):Play()
			end
		end)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Title.Size = UDim2.new(1, -70, 0, 20) -- Reduced width to accommodate keybind buttons
			Button.Visible = true
			Button.Parent = TabPage

			-- Keybind Button Injection for Buttons
			local KeybindButton = Instance.new("TextButton")
			KeybindButton.Name = "KeybindButton"
			KeybindButton.Size = UDim2.new(0, 20, 0, 20)
			KeybindButton.Position = UDim2.new(1, -40, 0.5, -10)
			KeybindButton.BackgroundColor3 = SelectedTheme.ElementBackground
			...
			ClearKeybindButton.Size = UDim2.new(0, 15, 0, 15)
			ClearKeybindButton.Position = UDim2.new(1, -60, 0.5, -7)
			ClearKeybindButton.BackgroundColor3 = SelectedTheme.ElementBackground
			ClearKeybindButton.BackgroundTransparency = 1
			ClearKeybindButton.Text = "x"
			ClearKeybindButton.TextColor3 = Color3.fromRGB(200, 50, 50)
			ClearKeybindButton.TextSize = 12
			ClearKeybindButton.Font = Enum.Font.GothamBold
			ClearKeybindButton.ZIndex = 51
			ClearKeybindButton.Visible = false
			ClearKeybindButton.Parent = Button
			...
			ButtonGradient.Parent = Button.Title

			TweenService:Create(Button, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
			TweenService:Create(Button.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			TweenService:Create(Button.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()	

			local function scaleButton(scale)
				TweenService:Create(Button, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1, scale, 0, 45)}):Play()
			end
			...
			Button.Interact.MouseButton1Click:Connect(triggerButton)
			...
			if ButtonSettings.Keybind ~= nil then
				KeybindButton.Text = tostring(ButtonSettings.Keybind)
				KeybindButton.TextColor3 = SelectedTheme.TextColor
				ClearKeybindButton.Visible = true
			end
			
			if settingsTable.General.keybindOptions.Value and not UserInputService.TouchEnabled then
			...
			ButtonSettings.KeybindButton = KeybindButton
			ButtonSettings.ClearKeybindButton = ClearKeybindButton
			
			ClearKeybindButton.MouseButton1Click:Connect(function()
				ButtonSettings.Keybind = nil
				KeybindButton.Text = "[ ]"
				KeybindButton.TextColor3 = Color3.fromRGB(240, 240, 240)
				ClearKeybindButton.Visible = false
				
				if ButtonSettings.Flag then
					local flagName = ButtonSettings.Flag .. "_Keybind"
					if RayfieldLibrary.Flags[flagName] then
						RayfieldLibrary.Flags[flagName].CurrentKeybind = nil
					end
				end
				if not ButtonSettings.Ext then
					SaveConfiguration()
				...
				inputConnection = UserInputService.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Keyboard then
						...
						if ButtonSettings.Flag then
							local flagName = ButtonSettings.Flag .. "_Keybind"
							if RayfieldLibrary.Flags[flagName] then
								RayfieldLibrary.Flags[flagName].CurrentKeybind = key
							end
						end
						if not ButtonSettings.Ext then
							SaveConfiguration()
			...
			-- Global Keybind Listener for Buttons
			local buttonKeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
				if not processed and ButtonSettings.Keybind and input.KeyCode.Name == ButtonSettings.Keybind then
					triggerButton()
				end
			end)
			table.insert(keybindConnections, buttonKeyConnection)

			-- Handle Theme Changes for Buttons
			Rayfield.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				KeybindButton.TextColor3 = SelectedTheme.TextColor
			end)

			-- Setup Flag for Button Keybind (If applicable)
			if ButtonSettings.Flag then
				local keybindFlag = ButtonSettings.Flag .. "_Keybind"
				local msg = {
					CurrentKeybind = nil,
					Type = "ButtonKeybind", -- Custom type to distinguish
					Flag = keybindFlag,
					ButtonSettings = ButtonSettings
				}
				
				-- Check cached settings
				if cachedSettings and cachedSettings[keybindFlag] then
					msg.CurrentKeybind = cachedSettings[keybindFlag].CurrentKeybind
					ButtonSettings.Keybind = msg.CurrentKeybind
					KeybindButton.Text = msg.CurrentKeybind
					KeybindButton.TextColor3 = SelectedTheme.TextColor
					if msg.CurrentKeybind then
						ClearKeybindButton.Visible = true
					end
				end

				function msg:Set(NewKeybind)
					ButtonSettings.Keybind = NewKeybind
					KeybindButton.Text = NewKeybind or "[ ]"
					KeybindButton.TextColor3 = SelectedTheme.TextColor
					if ClearKeybindButton then
						ClearKeybindButton.Visible = (NewKeybind ~= nil)
					end
				end

				RayfieldLibrary.Flags[keybindFlag] = msg
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			Dropdown.Selected.Font = Enum.Font.GothamMedium
			...
			DropdownGradient.Parent = Dropdown.Title
			...
			local function scaleDropdown(scale)
				local targetHeight = (Dropdown.List.Visible and not DropdownSettings._isClosing) and 180 or 45
				TweenService:Create(Dropdown, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(1, scale, 0, targetHeight)}):Play()
			end
			...
			Dropdown.Interact.MouseButton1Click:Connect(function()
				...
				local success, err = pcall(function()
					...
					if Dropdown.List.Visible then
						DropdownSettings._isClosing = true
						TweenService:Create(Dropdown, TweenInfo.new(0.3, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 45)}):Play()
						for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
							if DropdownOpt.ClassName == "Frame" and DropdownOpt.Name ~= "Placeholder" and DropdownOpt.Name ~= "SearchBar" then
								TweenService:Create(DropdownOpt, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {BackgroundTransparency = 1}):Play()
								if DropdownOpt:FindFirstChild("UIStroke") then
									TweenService:Create(DropdownOpt.UIStroke, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
								end
								if DropdownOpt:FindFirstChild("Title") then
									TweenService:Create(DropdownOpt.Title, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {TextTransparency = 1}):Play()
								end
							end
						end
						TweenService:Create(Dropdown.List, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ScrollBarImageTransparency = 1}):Play()
						TweenService:Create(Dropdown.Toggle, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Rotation = 180}):Play()	
						task.wait(0.3)
						Dropdown.List.Visible = false
						DropdownSettings._isClosing = false
					else
						TweenService:Create(Dropdown, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Size = UDim2.new(1, -10, 0, 180)}):Play()
						...
						TweenService:Create(Dropdown.Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Rotation = 0}):Play()
				...
				if not success then
					warn("Rayfield | Dropdown Click Error: " .. tostring(err))
					Dropdown.List.Visible = false -- Safety close
				end
			...
			Dropdown.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.Touch and not Dropdown.List.Visible then
					scaleDropdown(-15)
				end
			end)
			...
			local function SetDropdownOptions()
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					local isVisible = Dropdown.List.Visible and not DropdownSettings._isClosing
					DropdownOption.BackgroundTransparency = isVisible and 0 or 1
					DropdownOption.UIStroke.Transparency = isVisible and 0 or 1
					DropdownOption.Title.TextTransparency = isVisible and 0 or 1
			...
			if Icons then
				local asset = getIcon("search")
				SearchIcon.Image = 'rbxassetid://'..asset.id
				SearchIcon.ImageRectOffset = asset.imageRectOffset
				SearchIcon.ImageRectSize = asset.imageRectSize
			else
				SearchIcon.Image = "rbxassetid://6031154871" -- Default search icon
			end
			...
			SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				for _, opt in ipairs(Dropdown.List:GetChildren()) do
					if opt:IsA("Frame") and opt.Name ~= "Placeholder" and opt.Name ~= "SearchBar" then
						local optName = tostring(opt.Name)
						local lowerOptName = string.lower(optName)
						-- Check for substring match (case insensitive) or empty query
						local match = (query == "") or (string.find(lowerOptName, query, 1, true) ~= nil)
						opt.Visible = match
					end
				end
			end)
			...
			function DropdownSettings:Set(NewOption)
				...
				for _, droption in ipairs(Dropdown.List:GetChildren()) do
					if droption.ClassName == "Frame" and droption.Name ~= "Placeholder" and droption.Name ~= "SearchBar" then
						if not table.find(DropdownSettings.CurrentOption, droption.Name) then
							droption.BackgroundColor3 = SelectedTheme.DropdownUnselected
						else
							droption.BackgroundColor3 = SelectedTheme.DropdownSelected
			...
			function DropdownSettings:Refresh(optionsTable: table) -- updates a dropdown with new options from optionsTable
				...
				-- Filter current options to remove ones that no longer exist
				local NewCurrentOption = {}
				for _, v in ipairs(DropdownSettings.CurrentOption) do
					if table.find(optionsTable, v) then
						table.insert(NewCurrentOption, v)
					end
				end
				DropdownSettings.CurrentOption = NewCurrentOption
				...
				-- Efficiently update frames
				local currentFrames = {}
				for _, option in ipairs(Dropdown.List:GetChildren()) do
					if option.ClassName == "Frame" and option.Name ~= "Placeholder" and option.Name ~= "SearchBar" then
						currentFrames[option.Name] = option
					end
				end

				-- Remove frames for options that no longer exist
				local optionsMap = {}
				for _, name in ipairs(optionsTable) do
					optionsMap[name] = true
				end

				for name, frame in pairs(currentFrames) do
					if not optionsMap[name] then
						frame:Destroy()
						currentFrames[name] = nil
					end
				end

				-- Re-sync or create new options
				for _, Option in ipairs(optionsTable) do
					local DropdownOption = currentFrames[Option]
					if not DropdownOption then
						-- Create new frame only if it doesn't exist
						DropdownOption = Elements.Template.Dropdown.List.Template:Clone()
						DropdownOption.Name = Option
						DropdownOption.Title.Text = Option
						DropdownOption.Parent = Dropdown.List
						DropdownOption.Visible = true
						...
						DropdownOption.Interact.MouseButton1Click:Connect(function()
							...
							if not Success then
								warn("Dropdown Callback Error: " .. tostring(Response))
							end
					...
					-- Update existing or new frame selection state
					if table.find(DropdownSettings.CurrentOption, Option) then
						DropdownOption.BackgroundColor3 = SelectedTheme.DropdownSelected
					else
						DropdownOption.BackgroundColor3 = SelectedTheme.DropdownUnselected
				...
				if sb then
					...
					if tb and tb.Text ~= "" then
						...
						for _, opt in ipairs(Dropdown.List:GetChildren()) do
							if opt:IsA("Frame") and opt.Name ~= "Placeholder" and opt.Name ~= "SearchBar" then
								local match = (string.find(string.lower(opt.Name), query, 1, true) ~= nil)
								opt.Visible = match
							end
						end
					end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Title.Size = UDim2.new(1, -145, 0, 20) -- Reduced width to accommodate more left-aligned keybinds
			Toggle.Visible = true
			...
			-- Keybind Button Injection
			local KeybindButton = Instance.new("TextButton")
			KeybindButton.Name = "KeybindButton"
			KeybindButton.Size = UDim2.new(0, 20, 0, 20)
			KeybindButton.Position = UDim2.new(1, -110, 0.5, -10) -- Moved further left (from -78)
			KeybindButton.BackgroundColor3 = SelectedTheme.ElementBackground
			KeybindButton.BackgroundTransparency = 1
			KeybindButton.Text = "[ ]"
			KeybindButton.TextColor3 = Color3.fromRGB(240, 240, 240)
			KeybindButton.TextSize = 11
			KeybindButton.Font = Enum.Font.GothamBold
			KeybindButton.ZIndex = 50 -- Ensure it's above the main toggle button
			KeybindButton.Visible = false
			...
			ClearKeybindButton.Name = "ClearKeybindButton"
			ClearKeybindButton.Size = UDim2.new(0, 15, 0, 15)
			ClearKeybindButton.Position = UDim2.new(1, -130, 0.5, -7) -- Moved further left (from -95)
			ClearKeybindButton.BackgroundColor3 = SelectedTheme.ElementBackground
			ClearKeybindButton.BackgroundTransparency = 1
			ClearKeybindButton.Text = "x"
			ClearKeybindButton.TextColor3 = Color3.fromRGB(200, 50, 50)
			...
			ClearKeybindButton.ZIndex = 51
			ClearKeybindButton.Visible = false
			ClearKeybindButton.Parent = Toggle

			if ToggleSettings.Keybind ~= nil then
				KeybindButton.Text = tostring(ToggleSettings.Keybind)
				KeybindButton.TextColor3 = SelectedTheme.TextColor
				ClearKeybindButton.Visible = true
			end
			...
			ToggleSettings.KeybindButton = KeybindButton
			ToggleSettings.ClearKeybindButton = ClearKeybindButton
			
			ClearKeybindButton.MouseButton1Click:Connect(function()
				ToggleSettings.Keybind = nil
				KeybindButton.Text = "[ ]"
				KeybindButton.TextColor3 = Color3.fromRGB(240, 240, 240)
				ClearKeybindButton.Visible = false
				
				if ToggleSettings.Flag then
					local flagName = ToggleSettings.Flag .. "_Keybind"
					if RayfieldLibrary.Flags[flagName] then
						RayfieldLibrary.Flags[flagName].CurrentKeybind = nil
					end
				...
				inputConnection = UserInputService.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Keyboard then
						...
						if key == "Backspace" or key == "Delete" then
							...
							ToggleSettings.Keybind = key
							KeybindButton.Text = key
							KeybindButton.TextColor3 = SelectedTheme.TextColor
							if settingsTable.General.keybindOptions.Value then
								ClearKeybindButton.Visible = true
							end
						end
						...
						if ToggleSettings.Flag then
							local flagName = ToggleSettings.Flag .. "_Keybind"
							if RayfieldLibrary.Flags[flagName] then
								RayfieldLibrary.Flags[flagName].CurrentKeybind = ToggleSettings.Keybind
							end
							if not ToggleSettings.Ext then
								SaveConfiguration()
							end
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						...
						-- Wrap in proxy to detect Keybind changes from LoadConfiguration
						local proxy = setmetatable({}, {
							__index = function(_, k)
								return ToggleSettings[k]
							end,
							__newindex = function(_, k, v)
								ToggleSettings[k] = v
								if k == "Keybind" or k == "CurrentKeybind" then
									if ToggleSettings.KeybindButton then
										local txt = tostring(v or "[ ]")
										if txt == "nil" then txt = "[ ]" end
										ToggleSettings.KeybindButton.Text = txt
										ToggleSettings.KeybindButton.TextColor3 = SelectedTheme.TextColor
										
										if ToggleSettings.ClearKeybindButton and settingsTable.General.keybindOptions.Value then
											ToggleSettings.ClearKeybindButton.Visible = (v ~= nil)
										end
									end
								end
							end,
							__tostring = function() return tostring(ToggleSettings) end
						})
						RayfieldLibrary.Flags[ToggleSettings.Flag] = proxy
					end
				end
			end
			
			-- Setup Flag for Keybind
			if ToggleSettings.Flag then
				local keybindFlag = ToggleSettings.Flag .. "_Keybind"
				local msg = {
					CurrentKeybind = nil,
					Type = "ToggleKeybind",
					Flag = keybindFlag,
				...
				function msg:Set(NewBind)
					msg.CurrentKeybind = tostring(NewBind)
					ToggleSettings.Keybind = tostring(NewBind)
					KeybindButton.Text = tostring(NewBind)
					KeybindButton.TextColor3 = SelectedTheme.TextColor
				end
				...
				if cachedSettings and cachedSettings[keybindFlag] then
					msg.CurrentKeybind = cachedSettings[keybindFlag].CurrentKeybind
					ToggleSettings.Keybind = msg.CurrentKeybind
					KeybindButton.Text = msg.CurrentKeybind
					KeybindButton.TextColor3 = SelectedTheme.TextColor
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Slider.Main.Progress.Position = UDim2.new(0, 2, 0, 2)
			...
			local ProgressCorner = Instance.new("UICorner")
			ProgressCorner.CornerRadius = Slider.Main.UICorner.CornerRadius
			...
			local function UpdateSliderProgress(Value)
				...
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(Percentage, -4, 1, -4)}):Play()
			...
			function SliderSettings:Set(NewVal)
				...
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.45, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = UDim2.new(Percentage, -4, 1, -4)}):Play()
...
Main.Search.Input:GetPropertyChangedSignal('Text'):Connect(function()
	...
	if #Query > 0 then
		if not Elements.UIPageLayout.CurrentPage:FindFirstChild('SearchTitle-fsefsefesfsefesfesfThanks') then 
			...
			searchTitle.Title.Font = Enum.Font.GothamBold
			searchTitle.Title.TextColor3 = SelectedTheme.ToggleEnabled or Color3.fromRGB(145, 90, 255)
			...
			TitleGradient.Rotation = 90
			TitleGradient.Parent = searchTitle.Title
	...
	for _, element in ipairs(Elements.UIPageLayout.CurrentPage:GetChildren()) do
		if element.ClassName ~= 'UIListLayout' and element.Name ~= 'Placeholder' and element.Name ~= 'SearchTitle-fsefsefesfsefesfesfThanks' then
			if element.Name == 'SectionTitle' then
				...
				local Match = string.lower(element.Name):find(string.lower(Query), 1, true) ~= nil
				if Match then
					element.Visible = true
					element.GroupTransparency = 1
					TweenService:Create(element, TweenInfo.new(0.2, Enum.EasingStyle.Cubic), {GroupTransparency = 0}):Play()
				else
...
-- Global Keybind Listener for Toggles
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard then
		for _, flagSettings in pairs(RayfieldLibrary.Flags) do
			if type(flagSettings) == "table" and flagSettings.Type == "ToggleKeybind" then
				if flagSettings.CurrentKeybind and input.KeyCode.Name == flagSettings.CurrentKeybind then
					local toggleSettings = flagSettings.ToggleSettings
